...
// Declare stuff
let recipeContainer = document.querySelector(".recipe_review");
let button = document.querySelector('#searchbutton');
let input = document.querySelector('#searchinput')


// we wanna declare a random recipe by default, so
...
function search() {
    ...
    // OK so now we have these recipes filtered and in console log.
    // now lets sort them by rating (high to low stars ofc)

    let sortedRecipes = filterRecipes.sort(compareRecipes);
// sort them using da compare func
// sort() takes a high and low value
    ...
    // for each recipe within sortedRecipes, put it in the html
...
//                 <div class="tag"><h3>dessert</h3></div>
//                 <h2>${recipe.name}</h2>
...
//                 class="rating"
//                 role="img"
//                 aria-label="Rating: ${recipe.rating} out of 5 stars">${'⭐'.repeat(recipe.rating)}${'☆'.repeat(5 - recipe.rating)}
//                 </span>
//                 <p>${recipe.description}</p>
//             </div>`
...
//     // puts the html onto the created article element, so it's faster
...
function recipesTemplate(recipe) {
    ...
    <div class="recipe-card">
        <img src=${recipe.image} alt=${recipe.description}>
        <div class="recipe_info">
            <div class="tag"><h3>${tagTemplate(recipe.tags)}</h3></div>
            <h2>${recipe.name}</h2>

            <p><strong>Prep Time:</strong> ${recipe.prepTime}<br> <strong>Cook Time:</strong> ${recipe.cookTime}<br> <strong>Yield:</strong>  ${recipe.recipeYield}</p>

            <span
                class="rating"
                role="img"
                aria-label="Rating: ${recipe.rating} out of 5 stars">${ratingTemplate(recipe.rating)}
            </span>
            <p>${recipe.description}</p>
        </div>
    </div>`
...
init();
// init for initalizing stuff on the page, in this case this function always displays a random recipe.
