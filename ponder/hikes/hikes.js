...
// I might write some comments throughout to help me improve my overall pseudo code writing.
...
// we wanna declare a random hike by default, so:
...
function search() {
    // grab the value of our input box (search input box)
    let hikeQuery = input.value;

    let filterHikes = hikes.filter(function(hike) {
        
        return(
            // grab the value (lowercased) if it includes the hike Query (lowercased too)
            // OR if it's in the description, OR in the tags.
    ...
    // OK so now we have these hikes filtered and in console log.
    // now lets sort them by difficulty (low to high)

    let sortedHikes = filterHikes.sort(compareHikes);
// sort them using the comparehike function
// sort() takes a low and high value
    ...
    // for each hike within sortedHikes, put it in the html
...
init();
// init for initalizing stuff on the page, in this case this function always displays a random hike.
